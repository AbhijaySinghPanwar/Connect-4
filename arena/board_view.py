...
def to_svg(board):
    ...
    # Generate a unique animation name per move to force browser re-trigger
    anim_name = f"dropPiece_{board.latest_x}_{board.latest_y}"
    ...
    # Add pieces — both red and yellow use the same reusable animation
    svg += ''.join(f'''
            <circle 
    ...
    # Use the unique animation name so both red and yellow always animate
    svg += f'''
    </svg>
</div>
<style>
    .new-piece {{
        animation: {anim_name} 0.5s cubic-bezier(0.95, 0.05, 1, 0.5);
    }}
    .new-piece-highlight {{
        animation: {anim_name} 0.5s cubic-bezier(0.95, 0.05, 1, 0.5);
    }}
